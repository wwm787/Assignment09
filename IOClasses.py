        ...
        try:
            ...
            with open(file_name[1], 'w') as file:
                for myCD in lst_Inventory:
                    tracks = myCD.cd_tracks  # myCD.cd_tracks type= <class 'list'>
                    myCD_id = myCD.cd_id  # myCD.cd_id type= <class 'int'>
                    for trk in tracks:
                        if trk is not None:  # trk is <class 'DataClasses.Track'>
                            record = '{},{}'.format(myCD_id, trk.get_record())
                            billLstTmp = trk.get_record().strip().split(",")
                            billLstTmp.insert(0, str(myCD_id))
                            billTmpStr = ",".join(billLstTmp)
                            billTmpStr += "\n"
                            file.writelines(str(billTmpStr))
    ...
    def load_inventory(file_name: list) -> list:
        ...
        try:
            ...
            with open(file_name[1], "r") as file:
                tmpFileInfoHolder = file.readlines()
                while incRowItem < len(tmpFileInfoHolder):
                    tmpFileInfoHolder[incRowItem] = tmpFileInfoHolder[incRowItem].strip().split(
                        ",")
                    cd = PC.DataProcessor.select_cd(
                        lst_Inventory, int(tmpFileInfoHolder[incRowItem][0]))
                    track = DC.Track(int(
                        tmpFileInfoHolder[incRowItem][1]), tmpFileInfoHolder[incRowItem][2], tmpFileInfoHolder[incRowItem][3])
                    cd.add_track(track)
                    incRowItem += 1
        except Exception as e:
            print('There was a general error!',
